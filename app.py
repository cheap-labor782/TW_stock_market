...
st.markdown("**作者：趴元** | 作品展示用")

# ====================== 側邊欄 ======================
with st.sidebar:
    st.header("🔍 查詢設定")
    
    stock_input = st.text_input("輸入台股代碼（例如：2330）", value="2330").strip()
...
if df is not None and not df.empty:
    ...
    # ====================== 主 K 線圖 ======================
    st.subheader(f"{stock_input} K 線圖")
    ...
    fig.update_layout(height=650, template="plotly_white", legend=dict(orientation="h", y=1.05))
    ...
    with col2:
        if show_macd:
            ...
            fig_macd.add_bar(x=df.index, y=df['Histogram'], name='Histogram')
            st.plotly_chart(fig_macd, use_container_width=True)

    if show_volume:
        st.subheader("成交量")
        fig_vol = go.Figure(go.Bar(x=df.index, y=df['Volume']))
